...
- `pythonw.exe` 无控制台运行、单实例保护、Windows 登录自启动。
...
安装登录启动项后，每次登录 Windows 都会静默启动后台守候，不需要在每次启动 Codex 后手动运行。
...
后台程序启动时不会读取额度。第一次有效交互才会连接官方 Codex App Server；持续悬停时最多每 30 秒刷新一次。
